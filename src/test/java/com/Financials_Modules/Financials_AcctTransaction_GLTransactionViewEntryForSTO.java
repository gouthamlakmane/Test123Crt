...
public class Financials_AcctTransaction_GLTransactionViewEntryForSTO extends Base {
	...
	public static void Acct_Trans_Val_ShipID_1(List<HashMap<String, String>> data, int ShipIDRow, String PrdID, String CostValue) {
		...
		WebdriverWait.findElement("name", "shipmentId").sendKeys(data.get(ShipIDRow).get("Column8"));
			...
			for (int g = 1; g <= acctTransEntryTable.size(); g++) {
				...
				if (g == 1) {
					...
					if (prdID.contains(data.get(t - 1).get(PrdID))) {
						...
						Assert.assertEquals(data.get(21).get("Column1"), glAcctName);
						Assert.assertEquals(data.get(21).get("Column2"), flag);
				...
				if (g == 2) {
					...
					if (prdID.contains(data.get(t - 1).get(PrdID))) {
						Assert.assertEquals(data.get(22).get("Column1"), glAcctName);
						Assert.assertEquals(data.get(22).get("Column2"), flag);
						String value = WebdriverWait.findElement("xpath", "//*[@id='content-main-section']/div[3]/div[2]/table/tbody/tr[1]/td[8]").getText()
										.replace("₹", "").replace(",", "");
						AcctTrans_ShipID_softAssert.assertThat(value).isEqualTo(data.get(t - 1).get(CostValue));
