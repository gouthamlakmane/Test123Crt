...
import com.Utils.CheckRoundOff;
			...
			for (int g = 1; g <= acctTransEntryTable.size(); g++) {
				...
				if (g == 1) {
					...
					if (prdID.contains(data.get(t - 1).get(PrdID))) {
						...
						double v=CheckRoundOff.compareRoundoffValue(value, data.get(t - 1).get(CostValue));
						if(v>roundingOffValue) {
...
//						AcctTrans_ShipID_softAssert.assertThat(value).isEqualTo(data.get(t - 1).get(CostValue));
	...
	public static void Acct_Trans_Val_AddtnlCostInv(List<HashMap<String, String>> data) {
		...
		// Changes done for TC11 flow
		String amount=amt.replace("₹", "").replace(",", "");
		AcctTrans_InvID_softAssert.assertThat(amount).as("total Credit values has a difference ").isEqualTo(data.get(40).get("Column9"));
	...
	public static void Acct_Trans_Val_PaymentID(List<HashMap<String, String>> data, int PayID_row) {
		...
		List<WebElement> acctTransEntryTable = WebdriverWait.findElements("xpath", "//*[@id='content-main-section']/div[4]/div[2]/table/tbody/tr");
		System.out.println(acctTransEntryTable.size());
		for (int g = 1; g <= acctTransEntryTable.size(); g++) {

			String glAcctName = WebdriverWait.findElement("xpath", "//*[@id='content-main-section']/div[4]/div[2]/table/tbody/tr[" + g + "]/td[2]").getText();

			String flag = WebdriverWait.findElement("xpath", "//*[@id='content-main-section']/div[4]/div[2]/table/tbody/tr[" + g + "]/td[5]").getText();

			if (g == 1) {

				Assert.assertEquals(data.get(30).get("Column5"), glAcctName);
				Assert.assertEquals(data.get(30).get("Column6"), flag);
				String value = WebdriverWait.findElement("xpath", "//*[@id='content-main-section']/div[4]/div[2]/table/tbody/tr[1]/td[8]").getText()
								.replace(",", "");
